import { API, getWeatherData } from './modules/api.js'
import {
...
const createWeatherListRecursive = (weatherData, container) => {
  if (!weatherData.length) return 
  const data = weatherData.shift() 
  ...
  createWeatherListRecursive(weatherData, container)
}

const updateWeatherList = (data) => {
  UI_ELEMENTS.TEMP_INFO_LIST.replaceChildren()
  const todayData = data.filter((item) => isToday(item.dt))
...
const updateCityListRecursive = (cityList, index = 0) => {
...
const renderForecast = (data) => {
  updateWeatherList(data.list)
}

const repeatRequest = () => {
  const city = getCurrentCity()
  getWeatherData(API.URL_WEATHER, city, renderWeather)
  getWeatherData(API.URL_FORECAST, city, renderForecast)
}
